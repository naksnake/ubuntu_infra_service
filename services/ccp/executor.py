...
def _run_onboard(job_id, spec, secret, log):
    ...
    try:
        ...
        # machine and the inventory agree from the start. Without this a node
        # keeps a default name like 'ubuntu' that nothing else answers to.
...
def _hostname_script(new_name):
    ...
    script proves the change — CCP's name must never disagree with the box."""
...
def _run_hostname(job_id, spec, log):
    ...
    if rc != 0 or f'CCP_HOSTNAME_OK {new_name}' not in out or actual != new_name:
        ...
        log.write(f'[ccp] inventory left unchanged: {node["name"]} — CCP never '
                  'records a name the node does not actually have\n')
        return 1
...
def _run_hwscan(job_id, spec, log):
    ...
    # Inventory name vs the box's real hostname: drift means the rack view
    # describes a name the machine doesn't answer to — say so plainly.
...
def _run_ansible(job_id, spec, log):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # is killed out-of-band. Long playbooks may pass their own budget in
        # spec['timeout'].
