...
_GRES_LINE = re.compile(r'^NodeName=(\S+)\s+Name=gpu\s+File=(\S+)', re.M)


def gres_devices(gres_conf):
    """{node: [device files]} declared in a CCP gres.conf. These are the files
    slurmd waits 20 s for at start-up and then exits ("can't stat gres.conf
    file") when one is absent — so the deploy verifies them before it changes
    anything."""
    out = {}
    for node, files in _GRES_LINE.findall(gres_conf or ''):
        m = re.fullmatch(r'(.*?)\[(\d+)-(\d+)\]', files)
        if m:
            base, lo, hi = m.group(1), int(m.group(2)), int(m.group(3))
            out[node] = [f'{base}{i}' for i in range(lo, hi + 1)]
        else:
            out[node] = [files]
    return out


# The generated slurm.conf uses proctrack/linuxproc and task/none — no cgroup
# feature at all — yet slurmd still autodetects cgroup/v2 at start-up, creates
# a slurmstepd scope over dbus and, when launched by systemd, re-parents itself
# inside its own unit (the one part of slurmd start-up that behaves differently
# under systemd than in a shell). When any step of that fails, slurmd exits
# with "Unable to initialize cgroup plugin". Telling it to leave cgroups alone
# makes the start deterministic; switch to task/cgroup + autodetect when
# ConstrainDevices/ConstrainCores are wanted.
CGROUP_CONF = '''# generated by CCP — matches proctrack/linuxproc + task/none in slurm.conf.
# Set autodetect here together with TaskPlugin=task/cgroup when GPU/core
# confinement is wanted.
...
{_yaml_block(gres_conf, 10)}
    ...
    # Without it a rebooted node has no /dev/nvidia* until something opens the
    # driver — and slurmd would then wait 20 s and exit at boot.
    - name: Keep the NVIDIA device files across reboots (nvidia-persistenced, when installed)
      ansible.builtin.systemd:
        name: nvidia-persistenced
        enabled: true
        state: started
      failed_when: false
      when: inventory_hostname in ccp_gres_nodes
'''
    gres_nodes = gres_devices(gres_conf)
    gpu_gate = '' if not gres_nodes else f'''
    # ── GPU device files ──────────────────────────────────────────────────
    # slurmd waits 20 s for every File= device in gres.conf and then EXITS
    # ("can't stat gres.conf file"), so the files must exist now — not just
    # when the hardware was scanned: a rebooted node has no /dev/nvidia*
    # until the driver is loaded. nvidia-smi loads the module and creates the
    # device files when the driver is installed, so the probe repairs that
    # common case by itself.
    - name: Probe GPU device files (loads the NVIDIA driver if installed)
      ansible.builtin.shell: |
        command -v nvidia-smi >/dev/null 2>&1 && nvidia-smi -L >/dev/null 2>&1
        ls /dev/nvidia[0-9]* 2>/dev/null | sort -V | tr '\\n' ' '
      args:
        executable: /bin/bash
      register: gpu_devs
      changed_when: false
    ...
    - name: Note which declared GPU device files are missing on this node
      ansible.builtin.set_fact:
        gpu_missing_here: "{{{{ ccp_gres_nodes[inventory_hostname] | difference(gpu_devs.stdout | default('') | split) | sort }}}}"
      when: inventory_hostname in ccp_gres_nodes

    - name: Fail when a declared GPU device file is missing on its node
      run_once: true
      ansible.builtin.assert:
        that:
          - (ansible_play_hosts | map('extract', hostvars, 'gpu_missing_here') | select('defined') | map('length') | sum) == 0
        success_msg: "every declared GPU device file exists"
        fail_msg: |
          These nodes are declared with GPUs, but the device files do not exist on them right now — slurmd would wait 20 s for each and then exit:
          {{% for h in ansible_play_hosts if hostvars[h]['gpu_missing_here'] | default([]) | length > 0 %}}  {{{{ h }}}}: missing {{{{ hostvars[h]['gpu_missing_here'] | join(', ') }}}} — present now: {{{{ hostvars[h]['gpu_devs']['stdout'] | default('', true) | trim | default('(none)', true) }}}}
          {{% endfor %}}
          WHY: the NVIDIA driver is not loaded on that node (it was when the hardware was scanned; a reboot without nvidia-persistenced leaves no /dev/nvidia*), or the GPU is gone.
          WHAT TO DO: fix the driver on that node so `nvidia-smi -L` lists the GPUs, then Deploy again — or, if the node should run without GPUs, rescan it (Discover), Generate config again and Deploy.
...
- name: Deploy Slurm (CCP built-in, Ubuntu/Debian slurm-wlm)
  ...
  vars:
    ...
    ccp_nl: "\\n"
    # node → GPU device files it is declared with (from gres.conf); each must
    # exist before slurmd may be started there
    ccp_gres_nodes: {json.dumps(gres_nodes)}
  tasks:
    # ── read-only preflight ───────────────────────────────────────────────
...
{_yaml_block(slurm_conf, 10)}
    ...
    - name: Write cgroup.conf (cgroups off — matches proctrack/linuxproc + task/none)
      ansible.builtin.copy:
        dest: /etc/slurm/cgroup.conf
        owner: root
        mode: "0644"
        content: |
{_yaml_block(CGROUP_CONF, 10)}
{gres_task}
